 ...
 */
#ifndef _CMD_PARAM_REQ_H_
#define _CMD_PARAM_REQ_H_
 ...
 */
typedef Bool (*TCmdParamReqProcessSendMsg)(void *receiver, UInt8 host_source, UInt8 ser_num, UInt8 cmd, UInt8* data, UInt32 data_len);

typedef struct TCmdParamReqProcessTag TCmdParamReqProcess;
struct TCmdParamReqProcessTag
{
PROTECTED
    TOfObject parent_;              ///< 继承的父类
    TOfVirtualMethod Destroy_;      ///< 析构函数
PUBLIC
    TCmdParamReqProcessSendMsg       method_msg_send_;     ///< 发送消息方法(调用者实现)
    void                            *receiver_;            ///< 接收者
PRIVATE
